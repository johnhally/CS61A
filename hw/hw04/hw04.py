...
def balanced(m):
    ...
    return total_mass(end(left(m))) * length(left(m)) == total_mass(end(right(m))) * length(right(m)) and balanced(
...
def max_path_sum(t):
    ...
    max_t = 0
    for branch in branches(t):
        highest = max_path_sum(branch) + label(t)
        max_t = max(highest, max_t)
