...
def boar_brawl(player_score, opponent_score):
    ...
    score = (3 * abs(opponent_score // 10 % 10 - player_score % 10)) if opponent_score >= 10 else (3 * (player_score % 10))
    return max(score , 1)
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for i in range(10):
        cur_average = averaged_dice(i+1, dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    cur = boar_brawl(score, opponent_score) + score
    return 0 if sus_points(cur) - score >= threshold else num_rolls  
    # END PROBLEM 11
