    ...
    def double(self):
        ...
        if self.is_container:
            if not self.is_double:
                self.damage *= 2
                self.is_double = True
            if self.ant_contained and not self.ant_contained.is_double:
                self.ant_contained.damage *= 2
                self.ant_contained.is_double = True

        elif not self.is_container and not self.is_double:
            self.damage *= 2
            self.is_double = True
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if other.is_container == False and not self.ant_contained:
            return True
        else:
            return False
        # END Problem 8a
